...
#Program File: upd_link_category.pl
...
foreach $filename(@sub_list)
{
 ...
 ($page_id,$keyword,$catid,$cnt) = split (/\t/,$_);
  $keyword =~ s/\s+$//;
  if (!$hash1{$keyword}){
 print "cat:$filename\t$page_id\t$hash1{$keyword}\t$hash2{$keyword}\t$cnt\t$keyword\n";}
 if (!$hash2{$keyword}){
 print "parent:$filename\t$page_id\t$hash1{$keyword}\t$hash2{$keyword}\t$cnt\t$keyword\n";}
