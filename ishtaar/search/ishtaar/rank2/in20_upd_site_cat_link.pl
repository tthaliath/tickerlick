...
$query3 = $dbh->prepare ( 'select link_id from link_cat_rank where site_id = ? and cat_id = ? order by link_cat_rank desc') || die $query3->errstr;
$query4 = $dbh->prepare ( 'update site_cat_rank set link_str = ? where site_id = ? and cat_id = ?') || die $query4->errstr;
#$query2->execute();
open (F,"site_cat.txt");
