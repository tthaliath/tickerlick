...
$dbh = DBI->connect('dbi:mysql:tickmaster','root','Neha*2005')
 ...
 my $ins_sql = "insert into report (ticker_id,report_flag) values ($row[0],'MBX')";
 $ret = $dbh->do($ins_sql); 
 $ins_sql = "insert into reporthistory (ticker_id,report_flag,report_date) values ($row[0],'MBX','$price_date')";
 $ret = $dbh->do($ins_sql);
}
 $sth->finish;
 $dbh->disconnect; 
