...
$dbh = DBI->connect('dbi:mysql:tickmaster','root','Neha*2005')
 ...
 $sql = "select a.ticker_id,a.ticker from tickermaster a, tickerprice b where a.ticker_id = b.ticker_id and b.price_date = ?  and (b.ema_diff_5_35 - b.ema_macd_5) < 0";
  ...
  while (@row4 = $sth4->fetchrow_array)
   {
     if ($row4[5] > 5)
     {
        ...
        #print "$ins_sql\n";
        $ret = $dbh->do($ins_sql); 
        $ins_sql = "insert into reporthistory (ticker_id,report_flag,report_date) values ($row4[0],'$rep_flag','$price_date')";
        #print "$ins_sql\n";
        $ret = $dbh->do($ins_sql);
     }
   }
