...
$sql = "select 'OS', c.ticker,c.comp_name, c.sector,a.price_date, a.close_price, (ema_diff_5_35 - ema_macd_5) as MACD,sma_3_3_stc_osci_14 as Stochastic,b.rsi_14 as RSI, dma_20_sd, (((dma_20 + (2 * dma_20_sd))) - ((dma_20 -  (2 * dma_20_sd)))) 'BOLLINGER', b.rsistoch_14 from tickerprice a, tickerpricersi b, tickermaster c  where a.price_date = '$day' and c.tflag = 'Y' and c.ticker_id = a.ticker_id and a.ticker_id = b.ticker_id and a.price_date = b.price_date  and rsi_14 < 30 union  select  'OB', c.ticker,c.comp_name, c.sector,a.price_date, a.close_price, (ema_diff_5_35 - ema_macd_5) as MACD,sma_3_3_stc_osci_14 as Stochastic,b.rsi_14 as RSI, dma_20_sd, (((dma_20 + (2 * dma_20_sd))) - ((dma_20 -  (2 * dma_20_sd)))) 'BOLLINGER', b.rsistoch_14 from tickerprice a, tickerpricersi b, tickermaster c  where a.price_date = '$day' and c.tflag='Y' and c.ticker_id = a.ticker_id and a.ticker_id = b.ticker_id and a.price_date = b.price_date  and rsi_14 > 70";
 # print "$sql\n";
 my ($flag) = 0;
   my $file = "/home/tthaliath/tickerlick/daily/signal/signalstoch-".$day."\.csv";
    print "$file\n";
     open(OUT,">$file");
      $sth = $dbh->prepare($sql);
...
my $msg = MIME::Lite->new(
    ...
    Subject => 'daily security list - Stochstic Only ',
    Type    => 'multipart/mixed',
);
my $Mail_msg = "$day\n\n";
$Mail_msg .= "Tickerlick - Signal Stochastic\n\n";
open (F,"<$file");
undef $/;
$Mail_msg .= <F>;
close (F);
