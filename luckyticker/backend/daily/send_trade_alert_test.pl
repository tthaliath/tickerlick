...
my $ins = 'insert into trade_signal_seq (seq,signal,ticker,quotetime) values(?,?,?,now())';
#print $sql;
#exit 1;
#
      ...
      $sth = $dbh->prepare($sql);
       ...
       $sth->execute or die "SQL Error: $DBI::errstr\n";
        while (@row = $sth->fetchrow_array) {
         ...
         $Mail_msg .= "<a href='http://www.tickerlick.com/cgi-bin/quote?q=$row[1]'>$row[0] - $row[1] - $row[2]</a><br>";
