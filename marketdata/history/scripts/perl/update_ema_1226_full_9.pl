...
use strict;
use warnings;
my (@row,$tickid,$offset,$dmaday,$offset12,$offset26,$tickeridmin,$tickeridmax,$ema_12,$ema_26,@row_diff,$price_date);
$tickeridmin = $ARGV[0];
...
my $dma26 = new DMA(26,$dbh);
 my $query ="select count(*) as cnt, ticker_id from tickerprice where ticker_id >= $tickeridmin and ticker_id <= $tickeridmax group by ticker_id";
