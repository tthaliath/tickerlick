...
sub setDMASDVOL10
{
 ...
 $query ="SELECT sqrt(avg(squared10)) AS avg_sqd, max(price_date) as pdate,count(1) as rec_cnt FROM (SELECT squared10, price_date FROM voltemp WHERE ticker_id = $tickid ORDER BY price_date DESC LIMIT $offset,$self->{dmadays}) AS abc;";
 #print "$query\n";
 ...
 $updatedma = "update voltemp set  sd10 = $row[0] where ticker_id = $tickid and price_date = '$row[1]'";
 #print "$updatedma\n";
...
sub setDMASDVOL20
{
  ...
  my ($tickid) = shift;
  my ($query,$updatedma,$sth,@row,$ret);
 $updatedma = "insert into stock_volatility_daily (ticker_id,price_date,close_price,squared20) select ticker_id,price_date,close_price , (close_price - dma_20) * (close_price - dma_20)  from tickerprice where ticker_id = $tickid and price_date >= '2011-10-18' order by price_date  asc";
 #print "$updatedma\n";
 $ret = $self->{dbh}->do($updatedma);
}
...
sub setDMASDVOL20SD
{
 ...
 $query ="SELECT sqrt(avg(squared20)) AS avg_sqd, max(price_date) as pdate,count(1) as rec_cnt FROM (SELECT squared20, price_date FROM stock_volatility_daily WHERE ticker_id = $tickid ORDER BY price_date DESC LIMIT $offset,$self->{dmadays}) AS abc;";
 #print "$query\n";
 $sth = $self->{dbh}->prepare($query);
 $sth->execute or die "SQL Error: $DBI::errstr\n";
 #print "$query\n";
 ...
 #print "$row[0]\t$row[1]]\t$row[2]\n";
 if ($self->{dmadays} != $row[2]){next;}
 $updatedma = "update stock_volatility_daily set  sd20 = $row[0] where ticker_id = $tickid and price_date = '$row[1]'";
 #print "$updatedma\n";
 $ret = $self->{dbh}->do($updatedma);
}
$offset--;
...
sub setDMASDVOL20Daily
{
  my ($self) = shift;
  my ($tickid) = shift;
  my ($price_date) = shift;
  my ($query,$updatedma,$sth,@row,$ret);
 $updatedma = "insert into stock_volatility_daily (ticker_id,price_date,close_price,squared20) select ticker_id,price_date,close_price , (close_price - dma_20) * (close_price - dma_20)  from tickerprice where ticker_id = $tickid and price_date >= '$price_date`' order by price_date  asc";
 #print "$updatedma\n";
 $ret = $self->{dbh}->do($updatedma);
}

sub setDMASDVOL20SDDaily
{
  my ($self) = shift;
  my ($tickid) = shift;
  my ($offset) = shift;
