...
sub setRSIStochasticHistory
{
  my ($self) = shift;
  my ($tickid) = shift;
  my ($query,$updatedma,$sth,@row,$ret,$stc,$offset,$flag,$maxrsi,$lowrsi);
  $flag = 1;
  $offset = 16;
 while ($flag)
{
   $query ="select count(*),max(price_date),max(rsi_14),min(rsi_14) from (select price_date,rsi_14 from tickerpricersi b where b.ticker_id = $tickid order by price_date  asc limit $offset,14) as abc";
  $sth = $self->{dbh}->prepare($query);
  $sth->execute or die "SQL Error: $DBI::errstr\n";
 #print "$query\n";
  while (@row = $sth->fetchrow_array) {
...
#StochRSI = (RSI - Lowest Low RSI) / (Highest High RSI - Lowest Low RSI)
 $updatedma = "update tickerpricersi set rsistoch_14 =  (rsi_14 - $row[3])/($row[2] - $row[3]) where ticker_id = $tickid and price_date = '$row[1]'";
 #print "$updatedma\n";
 $ret = $self->{dbh}->do($updatedma);
 }
 $sth->finish;
...
sub setRSIStochasticDaily
{
  my ($self) = shift;
  my ($tickid) = shift;
  my ($query,$updatedma,$sth,@row,$ret,$stc,$offset,$flag,$maxrsi,$lowrsi);
   $query ="select count(*),max(price_date),max(rsi_14),min(rsi_14) from (select price_date,rsi_14 from tickerpricersi b where b.ticker_id = $tickid order by price_date  desc limit 0,14) as abc";
  $sth = $self->{dbh}->prepare($query);
  $sth->execute or die "SQL Error: $DBI::errstr\n";
 #print "$query\n";
  while (@row = $sth->fetchrow_array) {
 #print "$offset\t$row[0]\t$row[1]]\t$row[2]\t$row[3]\n";
 if ($self->{dmadays} != $row[0]){last;}
#StochRSI = (RSI - Lowest Low RSI) / (Highest High RSI - Lowest Low RSI)
 $updatedma = "update tickerpricersi set rsistoch_14 =  (rsi_14 - $row[3])/($row[2] - $row[3]) where ticker_id = $tickid and price_date = '$row[1]'";
 #print "$updatedma\n";
 $ret = $self->{dbh}->do($updatedma);
