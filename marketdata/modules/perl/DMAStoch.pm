...
sub setStochasticHistory
{
 ...
 while ($flag)
{
   $query ="select count(*),max(seq),max(high_price_14),min(low_price_14) from (select seq,high_price_14,low_price_14 from tickerrtq7 b where b.ticker_id = $tickid and price_date = '$pdate' order by seq asc limit $offset,14) as abc;";
  $sth = $self->{dbh}->prepare($query);
  $sth->execute or die "SQL Error: $DBI::errstr\n";
 #print "$query\n";
  while (@row = $sth->fetchrow_array) {
...
sub setDMAStoch
{
  ...
  my $countquery = "select count(*) from tickerrtq7 where ticker_id = $tickid and price_date = '$pdate'";
  ...
  while ($offset >= 0)
 {
 $query = "SELECT avg(stc_osci_14) AS avg_stc, max(seq) as pdate,count(1) as rec_cnt FROM (SELECT stc_osci_14, seq FROM tickerrtq7 WHERE ticker_id = $tickid and price_date = '$pdate' ORDER BY seq DESC LIMIT $offset,$self->{dmadays}) AS abc;";
...
sub setDMAStochFull
{
  ...
  while ($offset >= 0)
 {
 $query = "SELECT avg(sma_3_stc_osci_14) AS avg_sma_stc, max(seq) as pdate,count(1) as rec_cnt FROM (SELECT sma_3_stc_osci_14, seq FROM tickerrtq7 WHERE ticker_id = $tickid ORDER BY seq DESC LIMIT $offset,$self->{dmadays}) AS abc;";
