...
sub transformtickerdetetf
{
   ...
   my ($difflow,$perdiffhigh,$perdifflow,$diff,$perdiff,$diffhigh);
    if ($LastTrade == 0) {next;}
   if ($vol =~ /(.*?)-(.*)/  )
    {
       $vol = $2;
...
sub getpricehistory
{
 ...
 my $ticker_id = shift;
 my $no_of_days = shift;
 my (@row,$signal,$crossoverlast,$rowhtml,$signal2,@rowhtml2);
 my ($signallast) = 0;
 my ($signallast2) = 0;
 my ($rowprev) = 0;
 my ($rowprev2) = 0;
