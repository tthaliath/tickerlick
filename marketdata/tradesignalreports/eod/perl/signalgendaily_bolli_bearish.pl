...
my $sql = "select c.ticker, c.comp_name, a.price_date,a.close_price,(ema_diff - ema_macd_9) as signalstrength, (ema_diff_5_35 - ema_macd_5) as signalstrength2,b.rsi_14 as rsi,sma_3_3_stc_osci_14,(4*dma_20_sd/dma_20) as bolli  from tickerprice a,  tickerpricersi b, tickermaster c  where c.ticker_id = a.ticker_id and a.ticker_id = b.ticker_id and a.price_date = b.price_date and dma_20 is not null and dma_20_sd is not null and a.ticker_id <= 14000 and a.price_date = '$today' and (4*dma_20_sd/dma_20) between 0.1 and 0.25 and b.rsi_14 > 70 and sma_3_3_stc_osci_14 > 70 and (a.close_price - (dma_20 - (2 * dma_20_sd))) > 0 and (a.close_price - (dma_20 + (2 * dma_20_sd))) > 0";

my $Mail_msg = "<html><head></head><body>";
$Mail_msg .= "<h2>Tickerlick - Bollinger Sell Stocks - $today</h2>";
  #print "$sql\n";
   my $file = "/home/tthaliath/tickerlick/daily/signal/signal_bolli_sell_".$today."\.csv";
   my $flag = 0;
    #print "$file\n";
     open(OUT,">$file");
...
$subject = "Tickerlick - Bollinger Sell Stocks as of ".$today;
