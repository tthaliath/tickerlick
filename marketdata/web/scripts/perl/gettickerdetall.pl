...
use strict;
use warnings;
my (%rephash,$sql,@row,$query,$sth,$dbh);
...
my   $tickdesc = "";
my $tickprofile = "";
my   $content = get("http://finance.yahoo.com/quotes/$ticker/view/dv?bypass=true&ltr=1");
if ($content)
{
$content =~ s/\n//g;
...
my ($diff,$diffhigh,$perdiffhigh,$perdiff,$difflow,$perdifflow);
...
sub getpricehistory
{
 ...
 my ($signal3,$signal5,$signalhtml5,$signal6,$signalhtml6,$signal7,$signalhtml7); 
 my (@arrdata3,$rowhtml3,@arrdata5,$rowhtml5,@arrdata6,$rowhtml6,@arrdata7,$rowhtml7);
 my (%crossoverhash) = ( 
                         Bullish => 'Buy',
                         Bearish  => 'Sell',
                         nochange => '&nbsp;',
...
my $rowhtml2 = "<td>$row[7]</td><td>$signalhtml2</td>";
