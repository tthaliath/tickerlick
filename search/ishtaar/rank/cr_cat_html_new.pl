...
#Program File: cr_cat_html_new.pl
...
$dbh = open_dbi();
$query1 = $dbh->prepare ( 'select cat_desc from category_master_web where cat_id = ? and disp_flag="Y"') || die $query1->errstr;
my ($databuf) = '';
$k =0;
...
$databuf .= '<br><a href="/cgi-bin/searchbycategory.pl?cid='.$cat_id.'"><b>'.$cat.'</b></a><br>';
#$databuf .= '<br><a href="/perl/searchbycategory.pl?cid='.$cat_id.'"><b>'.$cat.'</b></a><br>';
foreach $subcat(split(/\,/,$cat_text))
{
  ...
  $databuf .= '<a href="/cgi-bin/searchbycategory.pl?cid='.$subcat.'">'.$cat_desc.'</a> &nbsp;';
}
}   
close F;
$databuf .= "</td>";
