...
my   $tickdesc = "";
my $tickprofile = "";
my ($response,$content,$contentprofile);
$response  = $browser->get("http://finance.yahoo.com/q?s=$ticker");
$content = $response->decoded_content;
if ($content)
{
$content =~ s/\n//g;
if ($content =~ m/There are no results for the given search term/)
{
...
else
{
 ...
 $contentprofile = $browser->get("http://finance.yahoo.com/q/pr?s=$ticker+Profile");
...
$contentprofile =~ s/\n//g;
$tickprofile = getetfprofile(\$contentprofile);
}
...
{
$contentprofile =~ s/\n//g;
$tickprofile = getprofile(\$contentprofile);
}
...
{
   my $str = shift;
   my ($diff,$wkRange,$perdiff,$diffhigh,$perdiffhigh,$difflow,$perdifflow);
   my (%tickerhash) = {};
...
sub transformtickerdetetf
{
   my $str = shift;
   my (%tickerhash) = {};
   my ($diff, $perdiff,$diffhigh,$perdiffhigh,$difflow,$perdifflow);
...
sub getpricehistory
{
 ...
 my ($signalhtml) = "";
 my ($signalhtml2) = "";
 my (@arrdata,@arrcrossover,@arrdata2,@arrcrossover2,@row);
 my (%crossoverhash) = ( 
                         Bullish => 'Buy',
                         Bearish  => 'Sell',
                         nochange => '&nbsp;',
...
my $rowhtml2 = "<td>$row[5]</td><td>$row[6]</td><td>$row[7]</td><td>$signalhtml2</td>";
...
my $dbh = DBI->connect('dbi:mysql:tickmaster','root','Neha*2005') or die "Connection Error: $DBI::errstr\n";
 my $sql = "select ticker_id from tickermaster where ticker = '$$tickerhashref{Ticker}'";
#print "tom1:$sql\n";
  #my $sth = $self->{dbh}->prepare($sql);
  my $sth = $dbh->prepare($sql);
 $sth->execute or die "SQL Error: $DBI::errstr\n";
