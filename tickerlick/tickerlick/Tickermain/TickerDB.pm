...
sub loadPrice
{
 ...
 #if price exists for given date. if exists, delete it first.
 #$deletesql = "delete from tickerpricersi where ticker_id = $ticker_id and price_date = '$self->{price_date}'";
 #$ret = $self->{dbh}->do($deletesql);
 #$insertsql = "insert into tickerpricersi (ticker_id,price_date,close_price) values ($ticker_id,'$self->{price_date}',$last_price)";
 #$ret = $self->{dbh}->do($insertsql);
}
