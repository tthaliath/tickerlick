...
my   $tickdesc = "";
my $tickprofile = "";
my $asset_class = "Common Stock";
my $betaflag = 0;
#my  $content = get("http://finance.yahoo.com/q?s=$ticker") || die "Timed out!";
my $content = getURLContent("http://finance.yahoo.com/q?s=$ticker");
#print "1\n";
if ($content)
{
#$content =~ s/\n//g;
#print "0000000000000000000000000:$content\n";
...
#print "test2\n";
$content =~ s/\n//g;
#$content =~ s/(.*)?Headlines<\/a>.*?More\sHeadlines?(.*)/$1$2/gs;
#print "$content\n";
#exit 1;
if ($content =~ m/There\sare\sno\sAll\sMarkets\sresults\sfor|Get\sQuotes\sResults\sfor/)
{
...
#Bid:</th><td class="yfnc_tabledata1">N/A</td></tr><tr><th scope="row" width="48%">Ask:</th><td class="yfnc_tabledata1">N/A</td>
#elsif ($content =~ m/Bid\:.*?<td class.*?>N\/A<\/td>.*?Ask\:.*?<td.*?>N\/A<\/td>/)
#{
# print "no:3\n";
# $tickhash{invalidticker} = 1;
# return %tickhash;
...
{
#Some ETN's are treated as equity.
$asset_class = 'ETF';
...
else
{
 ...
 #$content = get("http://finance.yahoo.com/q/pr?s=$ticker+Profile");
  $content = getURLContent("http://finance.yahoo.com/q/pr?s=$ticker+Profile");
  #print "2\n";
...
#elsif ($content =~ m/Beta\:/ )
#{
#   $asset_class = "Fund";
#   if ($content =~ m/52wk Range\:<\/th><td class\=\"yfnc_tabledata1\">N\/A<\/td>/)
#   {
#     $tickmain = getmainfund2(\$content);
#   }
#   else
#   {
#    $tickmain = getmainfund1(\$content);
#   }
#}
else
{
...
$tickprofile = getprofile(\$content);
  ...
  $content = getURLContent("http://finance.yahoo.com/q/ks?s=$ticker+Key+Statistics");
  #print "3\n";
  if ($content)
  {
  $content =~ s/\n//g;
...
#print "$tickdesc\n";
 ...
 {
     print "undefined keystat:$ticker\n";
     #return null;
 }
 }
 else
 {
     print "undefined profile:$ticker\n";
     #return null;
 }
 if ($tickmain ne 'NOMARKETCAP')
{
    #print "tickprofile:$tickprofile\n";
    #print "tickmain:$tickmain\n";
    #print "tickdesc:$tickdesc\n";   
    #$tickdata = $ticker."\t".$tickprofile."\t".$tickmain."\t".$tickdesc;
     #print "$tickdata\n";
     #$tickdata = $ticker."\t".$tickprofile."\t".$tickmain;
     #$tickdata = $ticker."\t".$tickmain;
     $tickdata = $ticker."\t".$tickmain."\t".$tickdesc;
 }
 }
 #print "tickdata:$tickdata\n";
 %tickhash =  transformtickerdet($tickdata,$asset_class);
 return %tickhash;
}
 ...
 sub getmain1
{
  ...
  my $content = $$contref;
   my($tickmain,$last);
   $tickmain = '';
  if ($content =~ m/time_rtq_ticker.*?<span id.*?>(.*?)<\/span>/)
   {
     $last = $1;
   if ($last == 0.00)
  {
...
if ($content =~ m/.*?time_rtq_ticker.*?><span.*?>(.*?)<\/span>.*?Prev Close.*?<td class.*?>(.*?)<\/td>.*?1y Target Est.*?<td class.*?>(.*?)<\/td>.*?52wk Range.*?<span.*?>(.*?)<\/span> - <span>(.*?)<\/span>.*?Volume.*?<span.*?>(.*?)<\/span>.*?Market Cap\:.*?<td class.*?>(.*?)<\/td>.*?P\/E .*?\(ttm\).*?<td class.*?>(.*?)<\/td>.*?EPS .*?\(ttm\).*?<td class.*?>(.*?)<\/td>.*?Div.*?Yield.*?<td class.*?>(.*?)<\/td>/)
 {
 #dont process if no market cap
 #if ($content =~ m/Market Cap:<\/th><td class\=\"yfnc_tabledata1\">N\/A<\/td>/) {return "NOMARKETCAP";}
#if ($content =~ m/.*?time_rtq_ticker.*?><span.*?>(.*?)<\/span>.*?1y Target Est.*?<td class.*?>(.*?)<\/td>.*?52wk Range.*?<span.*?>(.*?)<\/span> - <span>(.*?)<\/span>.*?Volume.*?<span.*?>(.*?)<\/span>.*?Market Cap\:.*?<span.*?>(.*?)<\/span>/)
...
if ($content =~ m/.*?time_rtq_ticker.*?><span.*?>(.*?)<\/span>.*?Prev Close.*?<td class.*?>(.*?)<\/td>.*?1y Target Est.*?<td class.*?>(.*?)<\/td>.*?52wk Range\:<\/th><td class\=\"yfnc_tabledata1\">(.*?)<\/td>.*?Volume.*?<span.*?>(.*?)<\/span>.*?Market Cap\:.*?<td class.*?>(.*?)<\/td>.*?P\/E .*?\(ttm\).*?<td class.*?>(.*?)<\/td>.*?EPS .*?\(ttm\).*?<td class.*?>(.*?)<\/td>.*?Div.*?Yield.*?<td class.*?>(.*?)<\/td>/)
 {
...
sub getprofile
{
  ...
  #print "$content\n";
  #exit 1;
#Sector:</td><td class="yfnc_tabledata1">N/A</td>
if ($content =~ m/<title>.*?\|(.*?)Stock.*?Sector:.*?<td.*?>N\/A<\/td>/)
{
...
elsif ($content =~ m/<title>.*?\|(.*?)Stock.*?Sector:.*?<a href.*?>(.*?)<\/a>.*?Industry:.*?<a href.*?>(.*?)<\/a>/)
{
...
if ($content =~ m/Yield/)
{
if ($content =~ m/Yield.*?td class.*?><yield>(.*?)<\/yield>/)
{

 if ($content =~ m/time_rtq_ticker.*?<span id.*?>(.*?)<\/span>.*?Prev Close.*?<td class.*?>(.*?)<\/td>.*?[Beta|NAV][\&|\:].*?td class.*?>(.*?)<\/td>.*?52wk Range.*?td class.*?><span>(.*?)<\/span> \- <span>(.*?)<\/span><\/td>.*?Volume.*?<span id.*?>(.*?)<\/span>.*?P\/E .*?\(ttm\).*?td class.*?>(.*?)<\/td>.*?Yield.*?td class.*?><yield>(.*?)<\/yield><\/td>/)
  {
...
{
if ($content =~ m/time_rtq_ticker.*?<span id.*?>(.*?)<\/span>.*?Prev Close.*?<td class.*?>(.*?)<\/td>.*?[Beta|NAV][\&|\:].*?td class.*?>(.*?)<\/td>.*?52wk Range.*?td class.*?><span>(.*?)<\/span> \- <span>(.*?)<\/span><\/td>.*?Volume.*?<span id.*?>(.*?)<\/span>.*?P\/E .*?\(ttm\).*?td class.*?>(.*?)<\/td>.*?Yield.*?<td class.*?>(.*?)<\/td>/)
  {
...
{
if ($content =~ m/time_rtq_ticker.*?<span id.*?>(.*?)<\/span>.*?Prev Close.*?<td class.*?>(.*?)<\/td>.*?[Beta|NAV][\&|\:].*?td class.*?>(.*?)<\/td>.*?52wk Range.*?td class.*?><span>(.*?)<\/span> \- <span>(.*?)<\/span><\/td>/)
#if ($content =~ m/time_rtq_ticker.*?<span id.*?>(.*?)<\/span>.*?1y Target Est.*?td class.*?>(.*?)<\/td>/)
{
      #$tickmain =  $1."\t".$2."\t".$3."-".$4."\t".$5."\t".$6."\t".$7;
      $tickmain =  $1."\t".$2."\t".$3."\t".$4."-".$5."\t"."0"."\t"."0"."\t0";
      $tickmain =~ s/<.*?>//g;
...
{
   my $str = shift;
   my $asset_class = shift;
   my (%tickerhash) = {};
   $tickerhash{PEGRatio}= 'NULL';
$tickerhash{OperatingCashFlow}= 'NULL';
$tickerhash{SharesOutstanding}= 'NULL';
$tickerhash{DivYield}= 'NULL';
$tickerhash{wkRange}= 'NULL';
$tickerhash{MostRecentQuarter}= 'NULL';
$tickerhash{dma10diffper}= 'NULL';
$tickerhash{QtrlyRevenueGrowth}= 'NULL';
$tickerhash{MarketCap}= 'NULL';
$tickerhash{WeekChange}= 'NULL';
$tickerhash{PrevClose}= 'NULL';
$tickerhash{dma50diffper}= 'NULL';
$tickerhash{difflowstat}= 'NULL';
$tickerhash{FiscalYearEnds}= 'NULL';
$tickerhash{LastTrade}= 'NULL';
$tickerhash{PayoutRatio}= 'NULL';
$tickerhash{EnterpriseValueEBITDA}= 'NULL';
$tickerhash{dma50stat}= 'NULL';
$tickerhash{GrossProfit}= 'NULL';
$tickerhash{perdifflow}= 'NULL';
$tickerhash{TrailingPE}= 'N/A';
$tickerhash{EnterpriseValue}= 'NULL';
$tickerhash{dma200}= 'NULL';
$tickerhash{TotalCashPerShare}= 'NULL';
$tickerhash{OperatingMargin}= 'NULL';
$tickerhash{PriceSales}= 'NULL';
$tickerhash{PriceStat}= 'NULL';
$tickerhash{diffhighstat}= 'NULL';
$tickerhash{RevenuePerShare}= 'NULL';
$tickerhash{pricediffper}= 'NULL';
$tickerhash{Beta}= 'NULL';
$tickerhash{WeekLow}= 'NULL';
$tickerhash{dma10stat}= 'NULL';
$tickerhash{NetIncomeAvltoCommon}= 'NULL';
$tickerhash{SharesShort}= 'NULL';
$tickerhash{PE}= 'N/A';
$tickerhash{QtrlyEarningsGrowth}= 'NULL';
$tickerhash{EnterpriseValueRevenue}= 'NULL';
$tickerhash{Ticker}= 'NULL';
$tickerhash{PriceBook}= 'NULL';
$tickerhash{SP50052WeekChange}= 'NULL';
$tickerhash{TotalDebtEquity}= 'NULL';
$tickerhash{dma200stat}= 'NULL';
$tickerhash{Nav}= 'NULL';
$tickerhash{dma50diff}= 'NULL';
$tickerhash{ReturnonAssets}= 'NULL';
$tickerhash{DilutedEPS}= 'NULL';
$tickerhash{pricediff}= 'NULL';
$tickerhash{Revenue}= 'NULL';
$tickerhash{exdividenddate}= 'NULL';
$tickerhash{ReturnonEquity}= 'NULL';
$tickerhash{dma10diff}= 'NULL';
$tickerhash{difflow}= 'NULL';
$tickerhash{TotalDebt}= 'NULL';
$tickerhash{class}= 'NULL';
$tickerhash{LeveredFreeCashFlow}= 'NULL';
$tickerhash{YearLow}= 'NULL';
$tickerhash{dma200diffper}= 'NULL';
$tickerhash{WeekHigh}= 'NULL';
$tickerhash{dma10}= 'NULL';
$tickerhash{ForwardPE}= 'N/A';
$tickerhash{dma50}= 'NULL';
$tickerhash{dma200diff}= 'NULL';
$tickerhash{yTargetEst}= 'NULL';
$tickerhash{EBITDA}= 'NULL';
$tickerhash{YearHigh}= 'NULL';
$tickerhash{twohundredDayMovingAverage}= 'NULL';
$tickerhash{CurrentRatio}= 'NULL';
$tickerhash{EPS}= 'NULL';
$tickerhash{diffhigh}= 'NULL';
$tickerhash{BookValuePerShare}= 'NULL';
$tickerhash{perdiffhigh}= 'NULL';
$tickerhash{fiftyDayMovingAverage}= 'NULL';
$tickerhash{TotalCash} = 'NULL';
   #tickdata:AAPL    Apple Inc.     Consumer Goods  Electronic Equipment    97.67   97.03   104.48  62.89-97.88     43,469,117      584.84B 16.39   5.96    1.88 (2.10%)    577.95B 16.39   13.89   1.26    3.26    4.81    3.24    9.78    Sep 28  Jun 28, 2014    28.84%  15.20%  31.56%  178.14B 28.81   6.00%   64.30B  59.13B  38.56B  5.96    12.30%  37.93B  6.33    31.04B  25.67   1.47    20.19   56.37B  37.43B  0.74    52.68%  17.39%  97.88   62.89   93.62   82.61   5.99B   16.81M  29.00%  May 8, 2014
...
#print "$str\n";
   ...
   elsif (   $wkRange =~ /(.*?)-(.*)/)
   {
       ...
       if ($tickerhash{YearHigh} < 0.01 || !$tickerhash{YearHigh} ){$tickerhash{$YearHigh} = 0.01;}
       ...
       if (!$tickerhash{YearLow} || $tickerhash{YearLow} < 0.01){$tickerhash{$YearLow} = 0.01;}
...
if ($MarketCap =~ /.*M$/)
{
   $MarketCap = $MarketCap * 1000 * 1000;
}
elsif ($MarketCap =~ /.*B$/)
{
   $MarketCap = $MarketCap * 1000 * 1000 * 1000;
}
