...
$query = "select ticker_id,ticker from ticker_master";
...
{
   my $lkey = lc $key;
   #print "$lkey\t$tickerhash{$key}\n";
   $keyhash{$lkey} = $key;
}
#$resulthtml .= $charthtml; 
#  my $tophtml .= "<tr><td><table><tr><td>Ticker&nbsp;:&nbsp;$tickerhash{Ticker}</td><td>Name&nbsp;:&nbsp;$tickerhash{name}</td></tr></table></td></tr>";
...
#if (not defined $tickerhash{Nav}){$tickerhash{Nav} = 'NULL';}
