...
if ($content =~ /.*?<span class\=\"volume last\-value\">(.*?)<\/span>.*?<span class\=\"last\-value\">(.*?)<\/span>.*?<span class\=\"text low\">(.*?)<\/span>.*?<span class\=\"text high\">(.*?)<\/span>.*?<span class\=\"text low\">(.*?)<\/span>.*?<span class\=\"text high\">(.*?)<\/span>/s)
{
   ...
   my @rank         = $t->findnodes('//*[@class="kv__label"]');
   my @overall_rank = $t->findnodes('//*[@class="kv__value kv__primary "]');
   my ($len) = scalar @rank;
   my ($i) = 0;
   while ($i < $len)
   {
   print $rank[$i]->as_text,$overall_rank[$i]->as_text,"\n";
   if ( $rank[$i]->as_text eq 'Market Cap') {$str->{'MarketCapitalization'} = $overall_rank[$i]->as_text;$i++;next;}
   if ( $rank[$i]->as_text eq 'P/E Ratio') {$str->{'PERatio'} = $overall_rank[$i]->as_text;$i++;next;}
   if ( $rank[$i]->as_text eq 'EPS') {$str->{'EarningsShare'} = $overall_rank[$i]->as_text;$i++;next;}
   if ( $rank[$i]->as_text eq 'Open') {$str->{'Open'} = $overall_rank[$i]->as_text;$i++;next;}
   $i++;
   }
   $v =~ s/\s+//g;
   if ($v =~ /.*K$/){$v *= 1000;}
