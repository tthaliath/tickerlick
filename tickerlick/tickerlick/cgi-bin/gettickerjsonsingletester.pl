...
my (%tick) = getResultsjson1($ARGV[0]);
sub getResultsjson1
{
my ($ticker) = uc shift;
my (%tickhash) ;
$sql = "select count(*) from tickermaster where ticker = '$ticker'";
...
#$Ticker,$LastTrade,$PrevClose,$yTargetEst,$wkRange,$vol,$MarketCap,$PE,$EPS
 %tickhash = transformtickerdetjson1($str,$str->{'Symbol'},$str->{'LastTradePriceOnly'},$str->{'PreviousClose'},$str->{'OneyrTargetPrice'},$str->{'Volume'},$str->{'YearLow'},$str->{'YearHigh'},$str->{'MarketCapitalization'},$str->{'PERatio'},$str->{'EarningsShare'});
...
else
{
 ...
 $sql = "select ticker_id,comp_name,sector,industry,tflag from tickermaster where ticker = $tickerhash->{Ticker}";
#print "tom1:$sql\n";
  $sth = $dbh->prepare($sql);
