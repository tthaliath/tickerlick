...
$sql = "select seq,rtq,dma_20, (dma_20 + (2 * dma_20_sd)) as dma_20_top,(dma_20 -  (2 * dma_20_sd)) as dma_20_bottom from (select a.seq,a.rtq,a.dma_20,a.dma_20_sd from tickerrtq7 a  where  ticker_id = ".$ticker_id." ORDER BY seq DESC LIMIT 0,200) as abc order by seq ASC";
#print "$sql\n";
 ...
 #push (@bigarr,@macd);
 my $maxmin = "select max((dma_20 + (2 * dma_20_sd))) as max,min((dma_20 -  (2 * dma_20_sd))) as min from  (select a.dma_20,a.dma_20_sd from tickerrtq7 a  where  ticker_id = ".$ticker_id." ORDER BY seq DESC LIMIT 0,200) as abc";
