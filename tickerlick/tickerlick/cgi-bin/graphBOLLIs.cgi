...
$sql = "select DATE_FORMAT(price_date, '%m/%d/%y') as price_date1,close_price,dma_20, (dma_20 + (2 * dma_20_sd)) as dma_20_top,(dma_20 -  (2 * dma_20_sd)) as dma_20_bottom from (select a.price_date,a.close_price,a.dma_20,a.dma_20_sd from tickerprice a  where  ticker_id = ".$ticker_id." and dma_20 is not null ORDER BY price_date DESC LIMIT 0,100) as abc order by price_date ASC";
#print "$sql\n";
 ...
 #push (@bigarr,@macd);
 my $maxmin = "select max((dma_20 + (2 * dma_20_sd))) as dmax,min((dma_20 -  (2 * dma_20_sd))) as dmin, max(close_price) pmax, min(close_price) pmin from  (select a.dma_20,a.dma_20_sd,close_price from tickerprice a  where  ticker_id = ".$ticker_id." and dma_20 is not null ORDER BY price_date DESC LIMIT 0,100) as abc";
