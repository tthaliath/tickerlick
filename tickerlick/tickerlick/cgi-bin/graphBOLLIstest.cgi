...
#print "$sql\n";
 ...
 #push (@bigarr,@macd);
 my $maxmin = "select max((dma_20 + (2 * dma_20_sd))) as dmax,min((dma_20 -  (2 * dma_20_sd))) as dmin, max(close_price) pmax, min(close_price) pmin from  (select a.dma_20,a.dma_20_sd,close_price from tickerprice a  where  ticker_id = ".$ticker_id." and a.dma_20 > 0 ORDER BY price_date DESC LIMIT 0,100) as abc";
