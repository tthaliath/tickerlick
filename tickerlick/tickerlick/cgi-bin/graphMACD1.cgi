...
$sql = "select DATE_FORMAT(price_date, '%m/%d/%y') as price_date1, ema_diff,ema_macd_9, (ema_diff - ema_macd_9) as signalstrength,price_date from (select a.price_date, a.ema_diff,a.ema_macd_9, (a.ema_diff - a.ema_macd_9) as signalstrength from tickerprice a where a.ticker_id =".$ticker_id." ORDER BY a.price_date DESC LIMIT 0,50) as abc where signalstrength is not null order by price_date ASC";
 $sth = $dbh->prepare($sql);
...
#Set the graph options.
  $graph->set( 
      ...
      title             => 'MACD (12,26,9) Crossover Chart',
      y_max_value       => $max,
      y_min_value       => $min,
      y_tick_number     => 5,
      y_label_skip      => 1,
