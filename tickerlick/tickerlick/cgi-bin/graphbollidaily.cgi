...
my ($price_date) = $q->param("dow");
my $maxticks = $q->param("maxtick");
my $x = "";
my $dma_20 = "";
my $signaltop = "";
my $rtq = "";
...
my $sql = "select '',rtq,dma_20, (dma_20 + (2 * dma_20_sd)) as dma_20_top,(dma_20 -  (2 * dma_20_sd)) as dma_20_bottom from (select a.seq,a.rtq,a.dma_20,a.dma_20_sd from tickerrtq7 a  where  ticker_id = ".$ticker_id." and price_date = '$price_date' and dma_20 is not null ORDER BY seq DESC) as abc order by seq ASC";
#print "$sql\n";
 ...
 $signalbottom =~ s/(.*)\,$/$1/;
 $signaltop =~ s/(.*)\,$/$1/;
 my $maxmin = "select max((dma_20 + (2 * dma_20_sd))) as max,min((dma_20 -  (2 * dma_20_sd))) as min from  (select a.dma_20,a.dma_20_sd from tickerrtq7 a  where  ticker_id = ".$ticker_id." and price_date = '$price_date' and a.dma_20 is not null) as abc";
