...
$sql = "select DATE_FORMAT(price_date, '%m/%d/%y') as price_date1,sd20 from (select a.price_date,a.sd20 from stock_volatility_daily a  where  ticker_id = ".$ticker_id." ORDER BY price_date DESC LIMIT 0,100) as abc order by price_date ASC";
#print "$sql\n";
 ...
 #push (@bigarr,@macd);
 my $maxmin = "select  max(sd20) pmax, min(sd20) pmin from  (select sd20 from stock_volatility_daily a  where  ticker_id = ".$ticker_id." ORDER BY price_date DESC LIMIT 0,100) as abc";
