...
print "start\n";
open (R, "<", "C:/Users/thaliath/workspace/tickercrawler/lastticker.txt") || die "Cannot open file lastticker for reading. $!\n";
while (<R>)
{
   chomp;
   print "$_\n";
...
open (IN,"<","C:/Users/thaliath/workspace/tickercrawler/usticker.txt") || die "Cannot open file usticker for reading. $!\n";
...
$content =~ s/\n//g;
open (com1, ">C:/Users/thaliath/workspace/tickercrawler/content.txt");
print com1 "$content\n\n\n";
close (com1);
#if ($content =~ m/.*?Last Trade.*?<span id.*?>(.*?)<\/span>.*?Trade Time.*?<span id.*?>(.*?)<\/span>.*?1y Target Est.*?td class.*?>(.*?)<\/td>.*?52wk Range.*?td class.*?><span>(.*?)<\/span> \- <span>(.*?)<\/span><\/td>.*?Market Cap\:.*?<span id.*?>(.*?)<\/span>.*?P\/E .*?\(ttm\).*?td class.*?>(.*?)<\/td>.*?EPS .*?\(ttm\).*?td class.*?>(.*?)<\/td>.*?Div.*?Yield.*?td class.*?>(.*?)<\/td>/i)  {
if ($content =~ /\>NAV\&|iShares/)
{
 print "$ticker\n";
open (ETF, ">>C:/Users/thaliath/workspace/tickercrawler/etf.txt");
print   ETF "$ticker\n";
close (ETF);
next;
}
...
#52wk Range:</th><td class="yfnc_tabledata1"><span>31.11</span> - <span>44.22</span></td></tr>
#52wk Range:</th><td class="yfnc_tabledata1"><span>8.94</span> - <span>17.70</span></td></tr><tr><th scope="row" width="48%">Volume:</th><td class="yfnc_ta
{
print "$ticker:getmain1\n";
$tickmain = getmain1(\$content);
}
else
{
 print "$ticker:getmain2\n";
 $tickmain = getmain2(\$content);
 }


     open (W,">>C:/Users/thaliath/workspace/tickercrawler/lastticker.txt");
      print W "$ticker\n";
      close (W);
        $tickerhash{$ticker}++;
      }
...
$tickprofile = getprofile(\$content);
 ...
 open (F,">>C:/Users/thaliath/workspace/tickercrawler/tickdata.txt");
 print F "\n$tickdata";
   close (F);
...
sub getmain2
{
  ...
  my ($tickmain);
  print "yes1\n";
if ($content =~ m/time_rtq_ticker.*?<span id.*?>(.*?)<\/span>.*?1y Target Est.*?td class.*?>(.*?)<\/td>.*?52wk Range.*?td class.*?><span>(.*?)<\/span> \- <span>(.*?)<\/span><\/td>.*?Volume.*?<span id.*?>(.*?)<\/span>.*?Market Cap\:.*?<td.*?>(.*?)<\/td>.*?P\/E .*?\(ttm\).*?td class.*?>(.*?)<\/td>.*?EPS .*?\(ttm\).*?td class.*?>(.*?)<\/td>.*?Div.*?Yield.*?td class.*?>(.*?)<\/td>/)
