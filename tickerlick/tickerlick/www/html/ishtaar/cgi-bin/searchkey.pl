...
use lib qw(/home/tthaliat/httpd/wwwdev/common);
$| = 1;
use strict;
use CGI;
my ($q) = new CGI;
...
$nolinks = $q->param("nl");
$keycnt = $q->param("kc");
$query_option = $q->param("q_0");
print "<br>ttt1:$pagetype";
print "<br>option:$query_option<br>sort:$sortby<br>links:$nolinks<br>size:$size<br>rowcount:$row_count\n";
#$querytext =~ s/\s/\\/g;
if ($pagetype eq 'MAIN' || $pagetype eq 'ADS') #The request is from main search
{
$querytext = $q->param("q");
@locarr = $q->param("l");
...
print "<br>ttt:$pagetype";
print "<br>option:$query_option<br>sort:$sortby<br>links:$nolinks<br>size:$size<br>rowcount:$row_count\n";
my (@key1,$key2,$key3);
if ($pagetype eq 'MAIN'){
if ($query_option == 3){ # exact phrase
$key2 = prepareKeywords($querytext,$query_option);
$key3 = prepareKeywordsExact($querytext);
checkresults($key2,$location,$offset,$row_count,$query_option,$sortby,$nolinks,$size,$key3);
}
else {
$key2 = prepareKeywords($querytext,$query_option);
#print "<br>dddddd:$key2\n";
checkresults($key2,$location,$offset,$row_count,$query_option,$sortby,$nolinks,$size);
}
}
 elsif ($pagetype eq 'RESULT'){
  # &getresults($catid,$keypat,$location,$res_count,$offset,$row_count);
   #print "<br>str:$catid<br>pat:$keypat<br>$res_count<br>$offset\n";
