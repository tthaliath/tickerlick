...
<img src="ishtaarlogo.png" width=100 height=45 alt="ISHTAAR"> 
<table><tr>
...
<br>
<p><br><br>
  <table width="550" border="0" cellpadding="0" cellspacing="0" align="left"><!--DWLayoutTable-->';
 
print $databufheader;
$key = $in{q};
($key1,$key2) = &prepareKeywords($key);
#my (@loclist) = split(/,/,$in{l});
my ($location) = join('|',split(/,/,$in{l}));
&getresults($key1,$key2,$location);
