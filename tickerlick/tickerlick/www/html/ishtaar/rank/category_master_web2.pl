...
open (F, "category_master_web2.txt") or die ("Could not open input file.");
open (OUT,">category_master_web3.txt") or die ("Could not open input file.");
while (<F>) {
chomp;
($cat_id,$cat_desc,$parent_id,$level,$cat_id_add,$disp_flag) = split (/\t/, $_);
$cat_str = '';
...
print "$i:$cat_id\t$cat_id_add\n";
   if ($cat_id_add =~ /\,/){
      foreach $val (split (/\,/,$cat_id_add)){
           ...
           if ($cat_str ne '') {$cat_str .= ",".$cathash{$val};}
           else{$cat_str = $cathash{$val};}
        }
   }
  else { $cat_str = $cathash{$cat_id_add};}
}
