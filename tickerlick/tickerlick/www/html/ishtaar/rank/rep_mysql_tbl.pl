...
$query2 = $dbh->prepare ( 'select link_id,site_id,site_loc_id, cat_id,link_cat_rank,site_rank_log from link_cat_rank where cat_id = "20-4"  order by site_rank_log desc, link_cat_rank desc' ) || die $query2->errstr;
