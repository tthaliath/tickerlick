...
#Program File: ut21_get_us_metros_business_cat.pl 
...
my ($i,$cat,$state,$metro,%crawled,$city,%orig,$busi,$base_link2,$base_link3,$base_link4);
my ($i) = 0;
...
open (CRAWLED1,"<us_metros_busi_cat.txt");
while (<CRAWLED1>){chomp; $crawled{$_}++;}
close (CRAWLED1);
open (NOCITY,">nometros_busi_cat.txt");
open (F,"us_metros_busi_list.txt");
while (<F>)
{
chomp;
my ($state,$metro,$busi)= split (/\t/,$_);
#print "$state\n";

$full_url = $base_link1.$state.$base_link2.$metro.$base_link3.$busi;
if ($crawled{$full_url}){next;}

$text = get $full_url;
#print "$full_url\n\n";
if (!$text){print NOCITY "$state\t$metro\t$busi\n";next}
$i++;
#$text =~ s/\n/ /g;
#$text =~ s/\s+/ /g;
open (OUT,">>us_metros_cat_list.txt");
if ($text =~ /.*?CATEGORIES(.*?)<\/table>/sgi){
 ...
 #print "CATEGORY:$1\n\n";
  ...
  while ($text1 =~ /.*?href.*?<b>(.*?)<\/b>/sgi){
     ...
     $cat =~ s/ /_/g;
     $cat =~ s/\,|\(|\)|\.|\/|\\/_/g;
     print OUT "$state\t$metro\t$busi\t$cat\n";
    }
}
close(OUT);

open (OUT1,">>us_metros_com_list.txt");
if ($text =~ /.*?SITE LISTINGS(.*?)<\/table>/sgi){
 #print "SITELISTIN:$1\n\n";
  ...
  while ($text2 =~ /.*?<li.*?href.*?\*(.*?)\">(.*?)<\/a>.*?<i>(.*?)<\/i>/sgi){
     ...
     $cloc = $3;
     $curl =~ s/\%3A/\:/gi;
     print OUT1 "$state\t$metro\t$busi\t$cname\t$curl\t$cloc\n";
    }
}
close(OUT1);

open (CRAWLED,">>us_metros_busi_cat.txt");
